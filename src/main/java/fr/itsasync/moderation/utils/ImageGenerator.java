...
import static fr.itsasync.moderation.utils.AsyncColors.*;
...
public class ImageGenerator {
    ...
    @SneakyThrows
    public BufferedImage generateLevelImage(Member member, AsyncMember asyncMember) {
        ...
        double advancement =
                (double) asyncMember.experience()
                        / (double) levelUtils.xpForNextLevelAt(asyncMember.level());
        g2d.setColor(accent);
        ...
        String levelContent = String.valueOf(asyncMember.level());
        ...
        String experienceContent =
                levelUtils.formatExperience(asyncMember.experience(), asyncMember.level());
        ...
        if (asyncMember.experience() == 0 && asyncMember.level() == 0) {
            rank = -1;
            rankText = "-";
        }
    ...
    public BufferedImage generateLeaderboardImage(
            int page,
            @Nullable AsyncMember user,
            List<AsyncMember> members,
            Function<AsyncMember, Member> memberMapper) {
        ...
        for (int i = 0; i < members.size(); i++) {
            ...
            if (discordMember == null) {
                AsyncMember asyncMember = members.get(i);
